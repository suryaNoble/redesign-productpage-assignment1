...
const FAQItem: React.FC<FAQItemProps> = ({
    ...
    <div className="border-b border-indigo-100">
        <button
            onClick={onClick}
            className="w-full py-3 px-4 flex items-center justify-between text-left transition-colors hover:bg-indigo-50 rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
            aria-expanded={isOpen}
            aria-controls={`faq-${question.replace(/\s+/g, '-')}`}
            id={`faq-${question.replace(/\s+/g, '-')}-header`}
        >
        ...
        <div
            ...
            role="region"
            aria-labelledby={`faq-${question.replace(/\s+/g, '-')}-header`}
...
const HomeFAQs: React.FC = () => {
    ...
    return (
        <section
            ...
            className="min-h-screen bg-gradient-to-br from-indigo-50 to-purple-50 py-16 px-4 sm:px-6 lg:px-8"
        >
